...
def create_hourly_graph(camera_id, selected_date=None):
    try:
        ...
        # Create hour bins for full 24 hours
        hours = list(range(24))
        ...
        title = f"Hourly Maximum Occupancy - {date_str}"
        
        # Format x-axis to show hours in 24-hour format
        ...
        fig = px.line(
            ...
            y='count',
            title=title,
            labels={'hour_str': 'Hour of Day (IST)', 'count': 'Maximum People Count'}
        )
...
def create_circular_graph(camera_id, selected_date=None):
    try:
        ...
        # Add title
        date_str = selected_date.strftime('%Y-%m-%d') if selected_date else "Last 24 Hours"
        title = f"Minute-by-Minute Presence - {date_str}"
...
with col2:
    ...
    st.write(f"Last updated: {current_time_ist.strftime('%H:%M:%S')} IST")
...
# Add note about persistence
st.markdown("---")
st.caption("Camera settings are saved in 'cameras.json' and occupancy data in 'occupancy_history.json'. All times are in Indian Standard Time (IST, UTC+5:30).")
